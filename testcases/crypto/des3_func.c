...
/*
 * Tests triple DES multipart encryption & multipart
 * decryption using generated keys.
 */
CK_RV do_EncryptDecryptUpdateDES3(struct generated_test_suite_info * tsuite)
{
    ...
    /** begin test **/
    testcase_begin("%s Multipart Encryption/Decryption tests with "
                   "key generation.", tsuite->name);
    testcase_rw_session();
    testcase_user_login();
    ...
    if (mech.mechanism != CKM_DES3_CBC_PAD) {
        ...
        while (k < original_len) {
            rc = funcs->C_EncryptUpdate(session, &crypt[k],
                                        DES3_BLOCK_SIZE, &crypt[k], &crypt_len);
            if (rc != CKR_OK) {
                testcase_error("C_EncryptUpdate rc=%s", p11_get_ckr(rc));
                goto error;
            }
...
/*
 * Tests triple DES encryption & decryption using
 * wrapped/unwrapped (generated) keys
 */
...
CK_RV do_SignVerifyMAC(struct published_mac_test_suite_info *tsuite)
{
    ...
    for (i = 0; i < tsuite->tvcount; i++) {
        ...
        case CKM_DES3_CMAC:
        case CKM_IBM_CMAC:
            mac_len = DES3_BLOCK_SIZE;
            break;
        case CKM_DES3_MAC:
            mac_len = DES3_BLOCK_SIZE / 2;
            break;
        default:
