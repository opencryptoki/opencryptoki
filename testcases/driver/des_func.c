...
CK_RV do_EncryptDES(struct published_test_suite_info *tsuite)
{
	...
	for (i = 0; i < tsuite->tvcount; i++){
		...
		rc = create_DESKey(session,
				...
				&h_key);
		if (rc != CKR_OK) {
			testcase_error("C_CreateObject rc=%s",
				p11_get_ckr(rc));
			goto testcase_cleanup;
		}
...
CK_RV do_EncryptUpdateDES(struct published_test_suite_info *tsuite)
{
	...
	for (i = 0; i < tsuite->tvcount; i++){
		...
		rc = funcs->C_EncryptFinal( session,
					    &actual[actual_len],
					    &k );
		if (rc != CKR_OK) {
			testcase_error("C_EncryptFinal rc=%s",
				p11_get_ckr(rc));
			goto error;
		}
...
CK_RV do_DecryptDES(struct published_test_suite_info *tsuite)
{
	...
	for (i = 0; i < tsuite->tvcount; i++){
		...
		/** begin test **/
		testcase_begin( "%s Decryption with published test vector "			"%d.", tsuite->name, i);
...
CK_RV do_EncryptDecryptUpdateDES(struct generated_test_suite_info *tsuite)
{
	...
	if (decrypt_len != original_len) {
		...
		testcase_pass("%s Multipart Encryption/Decryption with key" 			"generation test passed.", tsuite->name);
	}
...
CK_RV do_WrapUnwrapDES(struct generated_test_suite_info *tsuite)
{
	...
	if (actual_len != expected_len) {
		...
		testcase_fail("decrypted data does not match plaintext "
			"data.");
		rc = CKR_GENERAL_ERROR;
	}

	else {
		testcase_pass("DES Wrap/UnWrap test for %s passed.",
			tsuite->name);
	}
