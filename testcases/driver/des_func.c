...
//
//
int do_EncryptDES_Multipart_ECB( void )
{
...
//
//
int do_EncryptDES_Multipart_CBC( void )
{
	CK_BYTE             original[BIG_REQUEST];
	CK_BYTE             crypt1  [BIG_REQUEST];
	CK_BYTE             crypt2  [BIG_REQUEST];
...
//
//
int do_EncryptDES_CBC_PAD( void )
{
	CK_BYTE             original[BIG_REQUEST];
...
//
//
int do_EncryptDES_Multipart_CBC_PAD( void )
{
	CK_BYTE             original[BIG_REQUEST];
...
//
//
int do_WrapUnwrapDES_CBC_PAD( void )
{
	CK_BYTE             original[BIG_REQUEST];
...
int des_functions()
{
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptCDMF_CBC failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptDES_ECB failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptDES_CBC failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptDES_CBC_PAD failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptDES_imultipart_ECB failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptDES_Multipart_CBC failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_EncryptDES_Multipart_CBC_PAD failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_WrapUnwrapDES_ECB failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_WrapUnwrapDES_CBC failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
	...
	if (!rc) {
		fprintf (stderr, "ERROR do_WrapUnwrapDES_CBC_PAD failed, rc = 0x%0x\n", rc);
		if (!no_stop)
			return rc;
	}
	GetSystemTime(&t2);
