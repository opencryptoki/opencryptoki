...
if [ $RC_P11SAK_PINOPT = 0 ]; then
	echo "* TESTCASE list-key pin-opt PASS Token pin option handling"
else
	echo "* TESTCASE list-key pin-option FAIL Token pin option handling"
fi

if [ $RC_P11SAK_PINENV = 0 ]; then
	echo "* TESTCASE list-key pin-env PASS Token pin option handling"
else
	echo "* TESTCASE list-key pin-env FAIL Token pin option handling"
fi

if diff -q $P11SAK_ALL_PINOPT $P11SAK_ALL_PINENV ; then
	echo "* TESTCASE list-key pin-both PASS Token pin opt/env output compare"
else
	echo "* TESTCASE list-key pin-both FAIL Token pin opt/env output compare"
fi
