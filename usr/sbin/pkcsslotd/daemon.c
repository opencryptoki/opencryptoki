...
BOOL SaveStartupDirectory(char *Arg0)
{
    ...
    if (getcwd(cwd, PathSize) == NULL) {
        Err = errno;
        DbgLog(DL0, "SaveStartupDirectory: getcwd returned %s (%d)",
               SysConst(Err), Err);
        return FALSE;
    }
    ...
    if ((StartDir = calloc(PathSize, sizeof(char))) == NULL) {
        Err = errno;
        DbgLog(DL0,
               "SaveStartupDirectory: Unable to allocate %d bytes of memory "
               "for storage of the CWD. %s (%d)\n",
               SysConst(Err), Err);
        exit(1);
    }
    ...
    /* This will handle the case where you were in directory foo,
     * and started the daemon as bar/daemon
     */

    /* FIXME: This will not work properly if the daemon was found by 
     * searching the PATH */

    /* Make a local copy of the string because dirname() modifies it's
     * arguments */
    strcpy(arg, Arg0);

    /* note that dirname("daemon") and dirname("./daemon") will return "." */
    dname = dirname(arg);
    if (strcmp(dname, ".") != 0) {
        /* there's a / in it... */
...
BOOL GetStartDirectory(char *Buffer, u_int32 BufSize)
{
    ...
    if (StartDir == NULL) {
        DbgLog(DL0,
               "GetStartDirectory: Function called before "
               "SaveStartupDirectory()");
        return FALSE;
    }
