...
DL_NODE *proc_connections = NULL;
static struct listener_info admin_listener;
DL_NODE *admin_connections = NULL;
#ifdef WITH_LIBUDEV
struct udev_mon udev_mon;
#endif
DL_NODE *pending_events = NULL;
unsigned long pending_events_count = 0;
    ...
    case ADMIN_RECEIVE_PAYLOAD:
        ...
        if (rc != 0) {
            if (rc == -ENOSPC) {
                ...
                conn->state = ADMIN_W4_EVENT_LIMIT;
                return 0;
            }
            return rc;
        }
...
static void admin_event_limit_underrun(struct admin_conn_info *conn)
{
    ...
    if (conn->state != ADMIN_W4_EVENT_LIMIT)
        return;
...
static int udev_mon_init(const char *subsystem, struct udev_mon *udev_mon)
{
    ...
    if (rc != 0) {
        err = errno;
        ErrLog("%s: Failed add udev_mod socket %d to epoll, errno %d (%s).",
